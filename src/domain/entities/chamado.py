...
class Chamado(Entity):
    ...
    def validar_transicao(
        self, novo_status: StatusChamado, perfil_usuario: PerfilUsuario, usuario_id: UUID
    ) -> None:
        """
        ...
        if novo_status not in self.status_atual.transicoes_permitidas():
            raise InvalidStatusTransitionException(self.status_atual.value, novo_status.value)
        ...
        if perfil_usuario == PerfilUsuario.SOLICITANTE:
            if self.solicitante_id != usuario_id:
                raise AuthorizationException("Você só pode alterar seus próprios chamados")

        if (
            self.status_atual == StatusChamado.RESOLVIDO
            and novo_status == StatusChamado.EM_ATENDIMENTO
        ):
    ...
    def alterar_status(
        ...
        if novo_status == StatusChamado.RESOLVIDO:
            ...
            and status_anterior == StatusChamado.RESOLVIDO
        ):
    ...
    def transicoes_disponiveis(
        self, perfil: PerfilUsuario, usuario_id: UUID
    ) -> list[StatusChamado]:
        """
