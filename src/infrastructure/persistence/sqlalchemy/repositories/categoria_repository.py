...
class CategoriaRepository(ICategoriaRepository):
    ...
    def get_by_nome(self, nome: str) -> Categoria | None:
        model = self._session.query(CategoriaModel).filter(
            CategoriaModel.nome == nome.strip()
        ).first()
        return model.to_entity() if model else None
    
    def nome_exists(self, nome: str, exclude_id: UUID | None = None) -> bool:
        query = self._session.query(CategoriaModel).filter(
            CategoriaModel.nome == nome.strip()
        )
        if exclude_id:
    ...
    def get_ativas(self) -> list[Categoria]:
        models = self._session.query(CategoriaModel).filter(
            CategoriaModel.ativa == True
        ).order_by(CategoriaModel.nome).all()
        return [m.to_entity() for m in models]
    
    def get_all_ordered(self) -> list[Categoria]:
        models = self._session.query(CategoriaModel).order_by(
            CategoriaModel.nome
        ).all()
        return [m.to_entity() for m in models]
