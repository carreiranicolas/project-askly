...
class HistoricoStatusRepository(IHistoricoStatusRepository):
    ...
    def get_by_chamado(self, chamado_id: UUID, order_asc: bool = True) -> list[HistoricoStatus]:
    ...
    def get_ultimo_by_chamado(self, chamado_id: UUID) -> HistoricoStatus | None:
        model = (
            self._session.query(HistoricoStatusModel)
            .filter(HistoricoStatusModel.chamado_id == chamado_id)
            .order_by(HistoricoStatusModel.alterado_em.desc())
            .first()
        )
    ...
    def get_by_usuario(self, usuario_id: UUID) -> list[HistoricoStatus]:
        models = (
            self._session.query(HistoricoStatusModel)
            .filter(HistoricoStatusModel.alterado_por_usuario_id == usuario_id)
            .order_by(HistoricoStatusModel.alterado_em.desc())
            .all()
        )
        return [m.to_entity() for m in models]
