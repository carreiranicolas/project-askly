...
class ChamadoRepository(IChamadoRepository):
    ...
    def get_by_solicitante(
        ...
        total = query.count()
        models = query.order_by(ChamadoModel.criado_em.desc()).offset(
            (page - 1) * per_page
        ).limit(per_page).all()
    ...
    def get_by_atendente(
        ...
        query = self._session.query(ChamadoModel).filter(
            ChamadoModel.atendente_id == atendente_id
        )
