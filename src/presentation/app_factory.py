    ...
    if csp:
        talisman.init_app(
            ...
            strict_transport_security=bool(
                app.config.get("TALISMAN_STRICT_TRANSPORT_SECURITY", False)
            ),
        )
    ...
    CORS(
        ...
        resources={
            r"/api/*": {
                "origins": [
                    o.strip()
                    for o in app.config.get("CORS_ORIGINS", "http://localhost:5000").split(",")
                ],
...
def register_error_handlers(app: Flask) -> None:
    ...
    def not_found(error):
        if is_api_request():
            return (
                jsonify({"error": {"code": "NOT_FOUND", "message": "Recurso não encontrado"}}),
                404,
            )
...
def register_context_processors(app: Flask) -> None:
    ...
    def inject_enums():
        return {
            ...
            "get_categorias": lambda: CategoriaModel.query.filter_by(ativa=True)
...
def register_cli_commands(app: Flask) -> None:
    ...
    def seed_command():
        ...
        from src.infrastructure.persistence.sqlalchemy.models import CategoriaModel, UsuarioModel
