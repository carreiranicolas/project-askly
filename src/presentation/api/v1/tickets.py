...
ticket_create = tickets_ns.model('TicketCreate', {
    'titulo': fields.String(required=True, description='Título do chamado', example='Computador não liga'),
    'descricao': fields.String(required=True, description='Descrição detalhada', example='Ao pressionar o botão, nada acontece'),
    'categoria_id': fields.String(required=True, description='ID da categoria'),
    'prioridade': fields.String(description='Prioridade (BAIXA, MEDIA, ALTA, CRITICA)', default='MEDIA'),
})

ticket_response = tickets_ns.model('TicketResponse', {
    ...
    'transicoes_disponiveis': fields.List(fields.String(), description='Status disponíveis para transição'),
})

change_status_model = tickets_ns.model('ChangeStatus', {
    'status': fields.String(required=True, description='Novo status', 
                           enum=['EM_ATENDIMENTO', 'AGUARDANDO_RETORNO', 'RESOLVIDO', 'FECHADO']),
    'motivo': fields.String(description='Motivo da alteração'),
})

assign_model = tickets_ns.model('AssignAttendant', {
    'atendente_id': fields.String(required=True, description='ID do atendente'),
})

comment_create = tickets_ns.model('CommentCreate', {
    'conteudo': fields.String(required=True, description='Texto do comentário'),
})

comment_response = tickets_ns.model('CommentResponse', {
    'id': fields.String(),
...
pagination_model = tickets_ns.model('Pagination', {
    'page': fields.Integer(),
...
ticket_list_response = tickets_ns.model('TicketListResponse', {
