...
class AssignAttendantUseCase:
    ...
    def execute(self, dto: AtribuirAtendenteDTO, usuario_atual: Usuario) -> ChamadoResponseDTO:
        """
        ...
        if not usuario_atual.pode_atribuir_chamados():
            raise AuthorizationException(
                "Você não tem permissão para atribuir chamados", required_permission="assign_ticket"
            )
        ...
        with self.unit_of_work:
            ...
            if atendente.perfil not in (PerfilUsuario.ATENDENTE, PerfilUsuario.ADMIN):
                raise ValidationException(
                    "O usuário selecionado não é um atendente", field="atendente_id"
                )

            if not atendente.ativo:
                raise ValidationException(
                    "O atendente selecionado está inativo", field="atendente_id"
                )
